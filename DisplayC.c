...
void alarme_buzzer(){
  gpio_set_irq_enabled_with_callback(BUTTON_A, GPIO_IRQ_EDGE_FALL, false, &temporizador_callback);
  ssd1306_fill(&ssd, 0);
...
void cronometro_callback(uint gpio, uint32_t events){
  uint32_t current_time = to_us_since_boot(get_absolute_time());
  if (current_time - last_time2 > 300000)
  {
    last_time2=current_time;
   if(gpio==BUTTON_B){
     start = !start;
...
void temporizador(){//A variável rec_pos vai definir o valor do temporizador
  no_menu=rec_pos=selected=0; //Vai ser usada pra determinar quando sair da função (selected)
...
void ajustar_hora(){//A variável rec_pos vai definir se está selecionado a hora ou minuto
  no_relogio=1;
  ajustando_hora=0;
  start=rec_pos=selected=0; //Selected determina quando sair da função
